...
import com.common.core.config.event.DomainListUpdateEvent;
...
public class GetConfigManager {
    ...
    private GetConfigManager() {
        ...
        timestamp = U.getPreferenceUtils().getSettingLong(ConfigPreferenKey.PREFERENCE_KEY_CONFIG_TIMESTAMP, 0);
    ...
    public static synchronized GetConfigManager getInstance() {
        ...
        if (timediff > INTERVAL) {
            ...
            if (U.getPreferenceUtils().hasKey(ConfigPreferenKey.PREFERENCE_KEY_CONFIG_JSON)) {
                sInstance.parseJsonConfig(U.getPreferenceUtils().getSettingString(ConfigPreferenKey.PREFERENCE_KEY_CONFIG_JSON, ""));
            } else {
                sInstance.getConfig();
            }
        }
    ...
    private CertificationItem getCertificationTypeDrawableLocal(int certificationType, boolean needNoCache) {
        ...
        if (defaultItem == null) {
            ...
            if (certificationType == UserInfoConstans.CERTIFICATION_TV) {
                defaultItem.certificationType = UserInfoConstans.CERTIFICATION_TV;
                defaultItem.certificationDrawable = U.app().getResources().getDrawable(R.drawable.certification_type_6);
                defaultItem.certificationDrawableLiveComment = U.app().getResources().getDrawable(R.drawable.certification_type_6);
            } else {
                defaultItem.certificationType = -1;
                defaultItem.certificationDrawable = U.app().getResources().getDrawable(R.drawable.certification_type_1_3);
                defaultItem.certificationDrawableLiveComment = U.app().getResources().getDrawable(R.drawable.certification_type_1_3);
            }

            defaultItem.certificationDrawableLiveComment.setBounds(10, 0, U.getDisplayUtils().dip2px(20) + 10, U.getDisplayUtils().dip2px(16));
            if (!needNoCache) {
    ...
    private Drawable getLevelSmallDrawableLocal(int lvl) {
        ...
        return U.app().getResources().getDrawable(R.drawable.lv_1_circular);
    }
    ...
    private LevelItem getLevelItemLocal(int lvl) {
        ...
        if (itemDefault == null) {
            ...
            itemDefault.min = -1;
            itemDefault.max = -1;
            itemDefault.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_1_circular);
            itemDefault.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_1);
            itemDefault.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12), U.getDisplayUtils().dip2px(12));
    ...
    private Disposable mGetConfigDisposable;

    public void getConfig() {
        if (mGetConfigDisposable != null && !mGetConfigDisposable.isDisposed()) {
            return;
        }
    ...
    public void parseJsonConfig(final String str) {
        ...
        if (mParseJsonConfigDisposable != null && !mParseJsonConfigDisposable.isDisposed()) {
            return;
        }

        Observable.create(new ObservableOnSubscribe<List<LevelItem>>() {
            @Override
            public void subscribe(ObservableEmitter<List<LevelItem>> emitter) throws Exception {
                        ...
                        try {
                            ...
                            try {
                                if (disStr.has(CONFIG_PERMIT_TIMES)) {
                                    ...
                                    U.getPreferenceUtils().setSettingLong(ConfigPreferenKey.PRE_KEY_PERMIT_LONG, time);
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KEY_PERMIT_TIMES, times);
                                }
                            ...
                            try {
                                if (disStr.has(CONFIG_PERMIT_POPUP_TIMES_NOTIFY)) {
                                    ...
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KEY_PERMIT_TIMES_NOTIFY, times);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            try {
                                if (disStr.has(CONFIG_PERMIT_POPUP_TIMES_BACKLIVE)) {
                                    ...
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PER_KEY_PERMIT_TIMES_BACKLIVE, times);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            try {
                                if (disStr.has(CONFIG_PERMIT_BACKLIVE_LONG)) {
                                    ...
                                    U.getPreferenceUtils().setSettingLong(ConfigPreferenKey.PER_KEY_PERMIT_LONG_BACKLIVE, time);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            try {
                                if (disStr.has(CONFIG_PERMIT_NOTIFY_LONG)) {
                                    ...
                                    U.getPreferenceUtils().setSettingLong(ConfigPreferenKey.PER_KEY_PERMIT_LONG_NOTIFY, time);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            try {
                                if (disStr.has(CONFIG_TICKET_EXCHANGE_TYPE)) {
                                    ...
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PER_KEY_PERMIT_LONG_NOTIFY, type);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
...
//                                            if (item.max >= 100) {
                                                item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12), U.getDisplayUtils().dip2px(12));
//                                            } else if (item.max >= 10) {
//                                                item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(13.5f), U.getDisplayUtils().dip2px(12));
//                                            } else {
//                                                item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(16), U.getDisplayUtils().dip2px(12));
//                                            }
                            ...
                            if (disStr.has(CONFIG_VIDEO_RATE)) {
                                ...
                                U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PREF_KEY_VIDEO_RATE, videoRate);
                            }
                            if (disStr.has(CONFIG_PRIVATE_ROOM_SIZE)) {
                                int n = disStr.getInt(CONFIG_PRIVATE_ROOM_SIZE);
                                if (n > 0) {
                                    U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PREFERENCE_KEY_PRIVATE_ROOM_SIZE, n + "");
                                }
                            }
                            ...
                            if (disStr.has(CONFIG_HOME_PAGE_GUIDE_COIN)) {
                                ...
                                U.getPreferenceUtils().setSettingFloat(ConfigPreferenKey.PRE_KEY_HOME_PAGE_GUIDE_COIN, guideCoin);
                            }
                            ...
                            U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PREF_KEY_CONVERGED, convergedData.toString());
                            try {
                            ...
                            if (disStr.has("sys_account")) {
                                String oldAccount = U.getPreferenceUtils().getSettingString(ConfigPreferenKey.PRE_KEY_SIXIN_SYSTEM_SERVICE_WHITE_LIST, "");
                                if (TextUtils.isEmpty(oldAccount) || !oldAccount.equals(disStr.optString("sys_account"))) {
                                    U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PRE_KEY_SIXIN_SYSTEM_SERVICE_WHITE_LIST, disStr.getString("sys_account"));
                            ...
                            try {
                                if (disStr.has(CONFIG_LINE_BITRATE)) {
                                    ...
                                    try {
                                        U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PREF_KEY_LINE_BITRATE, Integer.parseInt(lineBitrate));
...
//                                U.getPreferenceUtils().setSettingString(GlobalData.app(), CONFIG_CHANNEL_NAME, channelName);
//                                U.getPreferenceUtils().setSettingString(GlobalData.app(), CONFIG_CHANNEL_URL, channelUrl);
...
//                                } else {
//                                    U.getPreferenceUtils().setSettingString(GlobalData.app(), CONFIG_CHANNEL_NAME, "");
//                                    U.getPreferenceUtils().setSettingString(GlobalData.app(), CONFIG_CHANNEL_URL, "");
...
//                            }
                            try {
                                if (disStr.has(CONFIG_BARRAGE_INTERVAL)) {
                                    ...
                                    if (!TextUtils.isEmpty(sendBarrageInterval))
                                        U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PRE_KEY_SEND_BARRAGE_INTERVAL, sendBarrageInterval);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            try {
                                if (disStr.has(CONFIG_KICK_PERMISSION_ANCHOR)) {
                                    ...
                                    if (!TextUtils.isEmpty(permission) && TextUtils.isDigitsOnly(permission)) {
                                        U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KIK_PERMISSION_ANCHOR, Integer.valueOf(permission));
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            ...
                            try {
                                if (disStr.has(CONFIG_KICK_PERMISSION_ADMIN)) {
                                    ...
                                    if (!TextUtils.isEmpty(permission) && TextUtils.isDigitsOnly(permission)) {
                                        U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KIK_PERMISSION_ADMIN, Integer.valueOf(permission));
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            ...
                            try {
                                if (disStr.has(CONFIG_KICK_PERMISSION_TOP1)) {
                                    ...
                                    if (!TextUtils.isEmpty(permission) && TextUtils.isDigitsOnly(permission)) {
                                        U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KIK_PERMISSION_TOP1, Integer.valueOf(permission));
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            ...
                            try {
                                if (disStr.has(CONFIG_LIVE_SCHEDULE_URL)) {
                                    ...
                                    if (!TextUtils.isEmpty(url)) {
                                        U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PREF_LIVE_SCHEDULE_URL, url);
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            ...
                            try {
                                if (disStr.has(CONFIG_FOLLOW_POPUP_USER_NUMBER)) {
                                    ...
                                    if (!TextUtils.isEmpty(userNum) && TextUtils.isDigitsOnly(userNum)) {
                                        U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_FOLLOW_POPUP_USER_NUMBER, Integer.valueOf(userNum));
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            ...
                            try {
                                if (disStr.has(CONFIG_FOLLOW_POPUP_VIEW_TIME)) {
                                    ...
                                    if (!TextUtils.isEmpty(viewTime) && TextUtils.isDigitsOnly(viewTime)) {
                                        U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_FOLLOW_POPUP_VIEW_TIME, Integer.valueOf(viewTime));
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            ...
                            try {
                                if (disStr.has(CONFIG_FOLLOW_CANCEL_TIME)) {
                                    ...
                                    if (!TextUtils.isEmpty(cancelTime) && TextUtils.isDigitsOnly(cancelTime)) {
                                        U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_FOLLOW_CANCEL_TIME, Integer.valueOf(cancelTime));
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            ...
                            try {
                                if (disStr.has(CONFIG_REDNAME_SENDSMS_INTERVAL)) {
                                    ...
                                    if (!TextUtils.isEmpty(interval)) {
                                        U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PRE_KEY_REDNAM_SENDSMS_INTERVAL, interval);
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                        ...
                        if (root.has(CONFIG_ZHIBO_AND_CONFIG)) {
                            ...
                            if (disStr.has(CONFIG_SCHEME_HOSTS)) {
                                ...
                                if (!TextUtils.isEmpty(schemeHosts)) {
                                    U.getPreferenceUtils().setSettingString(CONFIG_SCHEME_HOSTS, schemeHosts);
                                    parseHostsData();
                                }
                            }
                        }
                        ...
                        if (root.has("icon_vip_user_badge_and")) {
                            ...
                            while (keys.hasNext()) {
                                ...
                                try {
                                    ...
                                    item.certificationDrawableLiveComment.setBounds(10, 0, U.getDisplayUtils().dip2px(20) + 10, U.getDisplayUtils().dip2px(16));
                    ...
                    try {
                        if (root.has(CONFIG_FEEDS_DISPLAY)) {
                            ...
                            // todo ??? 可能有问题
                            String value = wallStr.getString("CN");
                        ...
                        if (root.has(CONFIG_ROOT_ZHIBO_COMMON)) {
                            ...
                            try {
                                if (comm.has(CONFIG_TICKET_EXCHANGE_TYPE)) {
                                    ...
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PER_KEY_TICKET_EXCHANGE_TYPE, type);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            try {
                                if (comm.has(CONFIG_GAME_FOLLOW)) {
                                    ...
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PER_KEY_GAME_SHOW_TIME, type);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            try {
                                if (comm.has(CONFIG_TICKET_EXCHANGE_NOTICE)) {
                                    ...
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PER_KEY_TICKET_EXCHANGE_NOTICE, type);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            ...
                            try {
                                if (comm.has(CONFIG_ENDLIVE_RECOMMEND_TIME)) {
                                    ...
                                    U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PER_KEY_ENDLIVE_RECOMMEND_TIME, time);
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            ...
                            if (!TextUtils.isEmpty(time)) {
                                U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PRE_KEY_STATISTICS_TRIGGER_TIME, time);
                            }
                            if (!TextUtils.isEmpty(sum)) {
                                U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PRE_KEY_STATISTICS_TRIGGER_SUM, sum);
                            }

                            MyLog.d("白名单 传过来的是" + comm.optInt(CONFIG_FAN_GROUP_GLOBAL_OPEN));
                            U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KEY_FANS_GROUP_GLOBAL_OPEN, comm.optInt(CONFIG_FAN_GROUP_GLOBAL_OPEN));

                            if (!TextUtils.isEmpty(comm.getString(CONFIG_FAN_GROUP_WHITE_LIST))) {
                                U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PRE_KEY_FANS_GROUP_WHITE_LIST, comm.getString(CONFIG_FAN_GROUP_WHITE_LIST));
                            }

                            int upgradeFlag = comm.getInt(CONFIG_UPGRADE_FLAG);
                            U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KEY_UPGRADE_FLAG, upgradeFlag);

                            int exchangeUseH5 = comm.getInt(CONFIG_COMMON_IS_EXCHANGE_H5);
                            U.getPreferenceUtils().setSettingInt(ConfigPreferenKey.PRE_KEY_EXCHANGE_USE_H5_FLAG, exchangeUseH5);

                            String exchangeH5Url = comm.getString(CONFIG_COMMON_EXCHANGE_H5_URL);
                            U.getPreferenceUtils().setSettingString(ConfigPreferenKey.PRE_KEY_EXCHANGE_H5_URL, exchangeH5Url);
                ...
                .subscribe(new Observer<List<LevelItem>>() {
                    ...
                    @Override
                    public void onComplete() {
                        mParseJsonConfigDisposable.dispose();
                        mParseJsonConfigDisposable = null;
                    }
    ...
    private void loadLocalResource() {
        {
            ...
            item.max = 5;
            int color = Color.parseColor("#58d4eb");
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_1_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_1);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_2_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_2);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12f), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            lvlDataListLocal.add(item);
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_3_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_3);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12f), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_4_1_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_4_1);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12f), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_4_2_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_4_2);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12f), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_4_3_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_4_3);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12f), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_5_1_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_5_1);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_5_2_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_5_2);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
            ...
            item.drawableBadge = U.app().getResources().getDrawable(R.drawable.lv_5_3_circular);
            item.drawableLevel = U.app().getResources().getDrawable(R.drawable.lv_5_3);
            item.drawableLevel.setBounds(0, 0, U.getDisplayUtils().dip2px(12), U.getDisplayUtils().dip2px(12));
            item.drawableBG = new RoundRectDradable(color);
            item.drawableBGType = LevelItem.JUST_COLOR;
            item.drawableBGColor = color;
        ...
        }
        {
            ...
            item.certificationDrawable = U.app().getResources().getDrawable(R.drawable.certification_type_1_3);
            item.certificationDrawableLiveComment = U.app().getResources().getDrawable(R.drawable.certification_type_1_3);

            item.certificationDrawableLiveComment.setBounds(10, 0, U.getDisplayUtils().dip2px(20) + 10, U.getDisplayUtils().dip2px(16));
            certificationListLocal.add(item);
            item = new CertificationItem();
            item.certificationType = 2;
            item.certificationDrawable = U.app().getResources().getDrawable(R.drawable.certification_type_2);
            item.certificationDrawableLiveComment = U.app().getResources().getDrawable(R.drawable.certification_type_2);
            item.certificationDrawableLiveComment.setBounds(10, 0, U.getDisplayUtils().dip2px(20) + 10, U.getDisplayUtils().dip2px(16));
            certificationListLocal.add(item);
            item = new CertificationItem();
            ...
            item.certificationDrawable = U.app().getResources().getDrawable(R.drawable.certification_type_4);
            item.certificationDrawableLiveComment = U.app().getResources().getDrawable(R.drawable.certification_type_4);
            item.certificationDrawableLiveComment.setBounds(10, 0, U.getDisplayUtils().dip2px(20) + 10, U.getDisplayUtils().dip2px(16));
            certificationListLocal.add(item);
        }
...
//        }
    ...
    public static Drawable getDrawableFromServer(String path, boolean isFromLevelDir) throws Exception {
        ...
        String name;
        if (isFromLevelDir) {
            name = U.getMD5Utils().MD5_16(path) + path.substring(path.lastIndexOf("."));
        } else {
            name = U.getMD5Utils().MD5_16(path);
        }
        File file = null;
        if (isFromLevelDir) {
    ...
    public String getMedalKey(String url) {
        ...
        String name = U.getMD5Utils().MD5_16(url);
...
//        medalAndIconPrefix = "http://zbtupian.zb.mi.com/staging_medal_and_icon_prefix_";
        ...
        if (drawable[0] == null) {
            if (mgetMedalSubscription != null) {
                return drawable[0];
            }
            Observable.create(
                    new ObservableOnSubscribe<Object>() {
//                        @Override
//                        public void (Subscriber<? super Object> subscriber) {
//                            try {
...
//                        }
    ...
    public List<Long> getSixinSystemServiceNumWhiteList() {
        String result = U.getPreferenceUtils().getSettingString(ConfigPreferenKey.PRE_KEY_SIXIN_SYSTEM_SERVICE_WHITE_LIST, "");
    ...
    private void parseHostsData() {
        String hosts = U.getPreferenceUtils().getSettingString(CONFIG_SCHEME_HOSTS, "");
    ...
    public static boolean canShowGroupIngress(long userId) {
        int isOpen = U.getPreferenceUtils().getSettingInt(ConfigPreferenKey.PRE_KEY_FANS_GROUP_GLOBAL_OPEN, 0);
        MyLog.d(TAG, "白名单isOpen=" + isOpen);
        if (isOpen == 1) {
            return true;
        } else {
            String whiteList = U.getPreferenceUtils().getSettingString(ConfigPreferenKey.PRE_KEY_FANS_GROUP_WHITE_LIST, "");
