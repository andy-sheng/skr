...
public abstract class BaseHolder<VM extends BaseViewModel> extends RecyclerView.ViewHolder {
    ...
    // 整体view的两边边距
    protected static final int SIDE_MARGIN = U.getDisplayUtils().dip2px(3.33f);
    // view内的中间边距
    protected static final int MIDDLE_MARGIN = U.getDisplayUtils().dip2px(1.33f);

    // 一排两个时， view内的中间边距
    protected static final int MIDDLE_MARGIN_TWO = U.getDisplayUtils().dip2px(6.67f);
    // 一排三个时， view内的中间边距
    protected static final int MIDDLE_MARGIN_THREE = U.getDisplayUtils().dip2px(5f);
    // 长图图片的宽高比
