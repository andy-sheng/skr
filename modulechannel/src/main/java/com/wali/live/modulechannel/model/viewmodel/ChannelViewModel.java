    ...
    protected void generateEncodeHead() {
        ...
        if (hasHead()) {
            ...
            if (!TextUtils.isEmpty(mHead)) {
                mHeadKey = U.getBase64Utils().encode(mHead.getBytes());
            }
            String encodeUri = "";
            if (!TextUtils.isEmpty(mHeadUri)) {
                encodeUri = U.getBase64Utils().encode(mHeadUri.getBytes());
            }
...
//            mStatisticsKey = String.format(StatisticsKey.KEY_CHANNEL_CLICK_MORE, mHeadKey, encodeUri);
    ...
    public void setImageSize(int width, int height) {
        ...
        if (mImageWidth != 0) {
            ...
            mFrameHeight = (int) (mRatio * U.getDisplayUtils().getPhoneWidth());
        } else {
            mFrameHeight = 0;
        }
    }
    ...
    protected static UserInfo buildBriefUser(com.wali.live.proto.CommonChannel.UserInfo brief) {
        if(brief == null) {
            return null;
        }
        ...
        userInfo.setUserId(brief.getZuid());
        userInfo.setAvatar(brief.getAvatar());
        userInfo.setUserNickname(brief.getNickname());
        userInfo.setLevel(brief.getLevel());
        userInfo.setCertificationType(brief.getCertificationType());
        return userInfo;
    }

    protected static UserInfo buildBriefUser(UserBrief brief) {
        if(brief == null) {
            return null;
        }
        ...
        userInfo.setUserId(brief.getUId());
        userInfo.setAvatar(brief.getAvatar());
        userInfo.setUserNickname(brief.getNickname());
        userInfo.setLevel(brief.getLevel());
        userInfo.setCertificationType(brief.getCertType());
        return userInfo;
    }
}
