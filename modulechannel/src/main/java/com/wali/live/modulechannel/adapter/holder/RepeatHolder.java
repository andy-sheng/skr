...
import static com.wali.live.modulechannel.model.viewmodel.ChannelLiveViewModel.RichText.LEFT_LABEL_BG;
...
public abstract class RepeatHolder extends FixedHolder {
    ...
    protected void bindShowModel(ChannelShowViewModel viewModel) {
        ...
        for (int i = 0; i < minSize; i++) {
            ...
            final ChannelShowViewModel.OneTextItem item = itemDatas.get(i);
            if (item != null) {
    ...
    protected void bindTwoTextModel(ChannelTwoTextViewModel viewModel) {
        List<ChannelTwoTextViewModel.TwoLineItem> itemDatas = viewModel.getItemDatas();
        int minSize = Math.min(mViewSize, itemDatas.size());
        setParentVisibility(minSize);
        for (int i = 0; i < minSize; i++) {
            mParentViews[i].setVisibility(View.VISIBLE);

            final ChannelTwoTextViewModel.TwoLineItem item = itemDatas.get(i);
            if (item != null) {
    ...
    protected void bindUserModel(ChannelUserViewModel viewModel) {
        List<ChannelUserViewModel.UserItemData> itemDatas = viewModel.getItemDatas();
        int minSize = Math.min(mViewSize, itemDatas.size());
        setParentVisibility(minSize);
        for (int i = 0; i < minSize; i++) {
            mParentViews[i].setVisibility(View.VISIBLE);
            final ChannelUserViewModel.UserItemData item = itemDatas.get(i);
            if (item != null) {
                AvatarUtils.loadAvatarByUrl(mImageViews[i], AvatarUtils.newParamsBuilder(item.getUser().getUserId()).setTimestamp(item.getUser().getAvatar()).setSizeType(ImageUtils.SIZE.SIZE_480).setCircle(true).build());
                mImageViews[i].setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        jumpItem(item);
                ...
                mTextViews[i].setText(item.getUser().getUserNickname());

                if (mBadgeIvs[i] != null) {
                    if (item.getUser().getCertificationType() > 0) {
                        mBadgeIvs[i].getLayoutParams().width = U.getDisplayUtils().dip2px(18f);
                        mBadgeIvs[i].getLayoutParams().height = U.getDisplayUtils().dip2px(12f);
                        //Todo-脚标暂时注释了
//                        mBadgeIvs[i].setImageDrawable(ItemDataFormatUtils.getCertificationImgSource(item.getUser().getCertificationType()));
                    } else {
                        mBadgeIvs[i].getLayoutParams().width = U.getDisplayUtils().dip2px(10.0f);
                        mBadgeIvs[i].getLayoutParams().height = U.getDisplayUtils().dip2px(10.0f);
                        //Todo-脚标暂时注释了
//                        mBadgeIvs[i].setImageDrawable(ItemDataFormatUtils.getLevelSmallImgSource(item.getUser().getLevel()));
    ...
    protected void bindItemOnUserModel(ChannelUserViewModel.UserItemData item, int i) {
    }
    ...
    protected void bindLiveModel(ChannelLiveViewModel viewModel) {
        ...
        for (int i = 0; i < minSize; i++) {
            ...
            if (item != null) {
                ...
                } else if (item instanceof ChannelLiveViewModel.UserItem) {
                    bindUserItem((ChannelLiveViewModel.UserItem) item, i);
                } else if (item instanceof ChannelLiveViewModel.VideoItem) {
                    bindVideoItem((ChannelLiveViewModel.VideoItem) item, i);
                } else if (item instanceof ChannelLiveViewModel.TVItem) {
                    bindTVItem((ChannelLiveViewModel.TVItem) item, i);
                } else if (item instanceof ChannelLiveViewModel.SimpleItem) {
                    bindSimpleItem((ChannelLiveViewModel.SimpleItem) item, i);
    ...
    protected void bindItemOnLiveModel(final ChannelLiveViewModel.BaseItem item, int i) {
    ...
    protected void bindUserItem(ChannelLiveViewModel.UserItem item, int index) {
    }
    ...
    protected void bindItemOnNavigateModel(ChannelNavigateViewModel.NavigateItem item, int i) {
     ...
     * @param i
     */
    protected void bindLeftLabel(final ChannelLiveViewModel.BaseItem item, int i) {
        ...
        if (!TextUtils.isEmpty(text)) {
            if(item.getTopLeft().hasBgColor()){
                ...
                float[] radius = {U.getDisplayUtils().dip2px(3.33f), U.getDisplayUtils().dip2px(3.33f), rightTop, rightTop, rightBottom, rightBottom, 0, 0 };
     ...
     */
    protected void bindLeftWidgetInfo(ChannelLiveViewModel.BaseItem item, int i) {
     ...
     */
    protected void bindLeftTopCorner(ChannelLiveViewModel.BaseItem item, int i) {
     ...
     * 展示门票直播
     */
    protected void bindMarkTv(ChannelLiveViewModel.BaseItem item, int i) {
