...
//            mTimeTvs[i] = $(mTimeTvIds[i]);
    ...
    protected int getImageWidth() {
        return U.getDisplayUtils().dip2px(101.33f);
    }
...
//                    true);
            AvatarUtils.loadAvatarByUrl(mAvatarIvs[i], AvatarUtils.newParamsBuilder(item.getUser().getUserId()).setTimestamp(item.getUser().getAvatar()).setCircle(true).setSizeType(ImageUtils.SIZE.SIZE_160).build());
            mNameTvs[i].setText(item.getUser().getUserNickname());
    ...
    protected void bindBaseLiveItem(ChannelLiveViewModel.BaseLiveItem item, int i) {
        ...
        mCountTvs[i].setText(U.app().getResources().getString(R.string.channel_read_count, item.getViewerCnt()));
        mCountTvs[i].setVisibility(View.VISIBLE);
    }

    @Override
    public void bindVideoItem(ChannelLiveViewModel.VideoItem item, int i) {
        mCountTvs[i].setText(U.app().getResources().getString(R.string.channel_read_count, item.getViewCount()));
        mCountTvs[i].setVisibility(View.VISIBLE);
    }
}
