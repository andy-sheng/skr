...
import com.wali.live.modulechannel.adapter.holder.EmptyHolder;
import com.wali.live.modulechannel.model.viewmodel.EmptyViewModel;

/**
 * Created by linjinbin on 16/3/2.
 *
 * @module 频道
 * @description 数据为空的抽象适配器
 */
public abstract class EmptyRecyclerAdapter extends RecyclerView.Adapter<BaseHolder> {
    ...
    public EmptyRecyclerAdapter() {
        mEmptyModel = new EmptyViewModel(U.app().getResources().getString(R.string.core_empty_tips),
    ...
    public BaseHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (mIsEmpty) {
            if (viewType == TYPE_EMPTY) {
                View view = LayoutInflater.from(U.app()).inflate(R.layout.channel_empty_view, parent, false);
