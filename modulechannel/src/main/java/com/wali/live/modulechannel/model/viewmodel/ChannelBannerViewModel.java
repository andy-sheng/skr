...
public class ChannelBannerViewModel extends ChannelViewModel<ChannelItem> {
    ...
    protected void parseTemplate(ChannelItem protoItem) throws Exception {
        ...
        parseUI(UiTemplateBanner.parseFrom(protoItem.getUiData().toByteArray()));
    }
...
//            mDescription = TestConstants.TEST_TITLE;
        ...
        public Banner(ChannelBanner protoBanner) {
            parse(protoBanner);
        }

        public Banner(BannerItemData protoBanner) {
            parse(protoBanner);
        }
        ...
        public void parse(ChannelBanner protoBanner) {
            if (protoBanner == null) {
                return;
            }
        ...
        public void parse(BannerItemData protoBanner) {
            if (protoBanner == null) {
                return;
            }
        ...
        protected void generateEncodeKey() {
            if (!TextUtils.isEmpty(mLinkUrl)) {
                try {
                    mEncodeKey = "banner_" + U.getBase64Utils().encode(mLinkUrl.getBytes("utf-8"));
