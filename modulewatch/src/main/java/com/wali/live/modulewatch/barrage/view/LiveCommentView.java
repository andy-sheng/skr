package com.wali.live.modulewatch.barrage.view;
...
import com.wali.live.modulewatch.cache.CommentLevelOrLikeCache;
...
import com.wali.live.modulewatch.event.UserActionEvent;
 ...
 * @module 直播间弹幕评论
 */
public class LiveCommentView extends RelativeLayout implements IComponentView<LiveCommentView.IPresenter, LiveCommentView.IView> {
    ...
    private static final int COMMENT_WIDTH_LANDSCAPE = U.getDisplayUtils().getPhoneHeight() / 2;
    private static final int COMMENT_HEIGHT_PORTRAIT = U.getDisplayUtils().dip2px(130.33f); // 弹幕区域高度
    private static final int COMMENT_HEIGHT_LANDSCAPE = U.getDisplayUtils().dip2px(60.33f);
    private static final int COMMENT_MARGIN_PORTRAIT = U.getDisplayUtils().dip2px(75f); // 弹幕区域右边距

    private static final int COMMENT_PADDING_LEFT = U.getDisplayUtils().dip2px(6);
    private static final int COMMENT_PADDING_RIGHT = U.getDisplayUtils().dip2px(6.67f);
    private static final int LANDSPACE_COMMENT_PADDING_BOTTOM = U.getDisplayUtils().dip2px(43.33f);
    private static final int PORTRAIT_COMMENT_PADDING_BOTTOM = U.getDisplayUtils().dip2px(6.66f);
        ...
        mAdapter.setLiveCommentNameClickListener(new LiveCommentRecyclerAdapter.LiveCommentNameClickListener() {
            ...
            public void onClickName(long uid) {
                if (null != mNameViewClickListener) {
                    ...
                    if (U.getCommonUtils().isFastDoubleClick(200)) {
                        return;
                    }
