...
package com.wali.live.modulewatch.barrage.view.adapter;
...
import com.wali.live.modulewatch.barrage.view.holder.LiveCommentHolder;
import com.wali.live.modulewatch.cache.CommentLevelOrLikeCache;
import com.wali.live.modulewatch.heart.HeartItemManager;
...
public class LiveCommentRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private final static int BACKGROUD_MARGIN = U.getDisplayUtils().dip2px(1);
    ...
    private void bindLevelView(LiveCommentHolder liveCommentHolder, CommentModel liveComment) {
        ...
        if (liveComment.isNoble() && level > 0) {
            ...
            if (level != 0
                    && liveComment.getSenderId() == MyUserInfoManager.getInstance().getUid()
...
//                            U.app().getString(R.string.game_give_one_gift, gift.getName()),
//                            liveComment.getCommentColor(), liveCommentHolder.getClickSpan(false));
//                } else {
//                    liveCommentHolder.setComment(bConfigList, aConfigList,
//                            U.app().getResources().getQuantityString(R.plurals.game_give_n_gift, count, count, gift.getName()),
//                            liveComment.getCommentColor(), liveCommentHolder.getClickSpan(false));
//                }
//
        ...
        if (liveComment.getMsgType() == BarrageMsgType.B_MSG_TYPE_LIKE) {
            ...
            if (!TextUtils.isEmpty(path)) {
                ...
                if (null == CommentLevelOrLikeCache.getLevelOrLike(path)) {
                    bitmap = U.getBitmapUtils().getLocalBitmap(path);
            ...
            if (drawable == null) {
                ...
                if (resId != 0) {
                    drawable = U.app().getResources().getDrawable(resId);
                }
            }
            if (drawable != null) {
                drawable.setBounds(0, 0, U.getDisplayUtils().dip2px(16f), U.getDisplayUtils().dip2px(16f));
            ...
            if ((type == BarrageMsgType.B_MSG_TYPE_TEXT
                ...
                if (mIsGameLive) {
                    ...
                    if (liveComment.getBackGround() == 0) {
                        liveCommentHolder.itemView.setBackground(U.app().getResources().getDrawable(R.drawable.live_bg_comment));
                    } else {
                        liveCommentHolder.itemView.setBackground(U.app().getResources().getDrawable(liveComment.getBackGround()));
                    }
                }
...
//            return new FootViewHolder(footView);
        ...
        if (mIsGameLive) {
            ...
            params.rightMargin = U.getDisplayUtils().dip2px(3.33f);
            params.topMargin = U.getDisplayUtils().dip2px(8.33f);
        }
