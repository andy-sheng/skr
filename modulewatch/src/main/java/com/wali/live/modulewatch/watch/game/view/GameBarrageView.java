...
import com.wali.live.modulewatch.utils.AnimationPlayControlTemplate;
...
public class GameBarrageView extends RelativeLayout
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        mFlyBarrageControl = new AnimationPlayControlTemplate<CommentModel>(
                (BaseActivity) getContext(), false, ROAD_NUM) {
            @Override
     ...
     * 每个item占用的高度
     */
    private int mPerHeight = U.getDisplayUtils().dip2px(30);
        ...
        for (int i = 0; i < ROAD_NUM; i++) {
            ...
            if (mRoadEnterNumber[i] == 0 && mRoadRunNumber[i] == 0) {
                ...
                if (model.getSenderId() == UserAccountManager.getInstance().getUuidAsLong()) {
                    fbViewInfo.view.setTextColor(Color.parseColor("#5fffd0"));
                } else {
                    fbViewInfo.view.setTextColor(getResources().getColor(R.color.white_trans_80));
                }
                playFly(fbViewInfo);
                return true;
            }
