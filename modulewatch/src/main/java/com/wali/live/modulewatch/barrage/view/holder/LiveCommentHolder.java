package com.wali.live.modulewatch.barrage.view.holder;
...
import com.common.core.config.GetConfigManager;
...
import com.wali.live.modulewatch.barrage.view.adapter.LiveCommentRecyclerAdapter;
import com.wali.live.modulewatch.cache.CommentNobelLevelCache;
import com.wali.live.modulewatch.cache.CommentVfansLevelCache;
import com.wali.live.modulewatch.cache.CommentVipLevelIconCache;
import com.wali.live.modulewatch.utils.NobleConfigUtils;
import com.wali.live.modulewatch.utils.SpanUtils;
import com.wali.live.modulewatch.utils.VfansInfoUtils;
import com.wali.live.modulewatch.utils.VipLevelUtil;
import com.wali.live.modulewatch.base.view.LevelIconsLayout;
...
//    }
    ...
    public void setNameSpan(List<String> beforeNickNameConfigList, List<String> afterNickNameConfigList, String name, @ColorRes int colorId,
        ...
        if (certificationType > 0) {
            ...
            if (certificationType > 0) {
                drawable = GetConfigManager.getInstance().getCertificationTypeDrawable(certificationType).certificationDrawable;
            }
...
//            drawable.setBounds(0, 0, (int) (drawable.getIntrinsicWidth() * U.getDisplayUtils().getDensity()), (int) (drawable.getIntrinsicHeight() * U.getDisplayUtils().getDensity()));//思敏老师的原代码
            int currentSdk = Build.VERSION.SDK_INT;
            int lineSpace = currentSdk >= Build.VERSION_CODES.LOLLIPOP_MR1 ? 0 : U.getDisplayUtils().dip2px(5);//行间距需求5.0版本适配
        ...
        if (!isNotTextBarrage) {
            ...
            commentSpan.setSpan(new ForegroundColorSpan(U.app().getResources().getColor(colorId)),
        ...
        if (isSell) {
            commentSpan.setSpan(new ForegroundColorSpan(U.app().getResources().getColor(R.color.barrage_sell_comment)),
    ...
    public void setLevelInfo() {
        ...
        if (pair.first == true) {
            ...
            TextView vipLevelView = LevelIconsLayout.getDefaultTextView(context == null ? U.app() : context);
            ...
            vipLevelDrawable.setBounds(0, 0, (int) (vipLevelDrawable.getIntrinsicWidth() * U.getDisplayUtils().getDensity()),
                    (int) (vipLevelDrawable.getIntrinsicHeight() * U.getDisplayUtils().getDensity()));
    ...
    public void setVfansMedalInfo() {
        if (!TextUtils.isEmpty(liveComment.getVfansMedal()) && liveComment.getVfansLevel() > 0
            ...
            levelDrawable.setBounds(0, 0, (int) (levelDrawable.getIntrinsicWidth() * U.getDisplayUtils().getDensity()),
                    (int) (levelDrawable.getIntrinsicHeight() * U.getDisplayUtils().getDensity()));
    ...
    private void updateCommonSpanInDrawable(Drawable spanDrawable) {
        int currentSdk = Build.VERSION.SDK_INT;
        int lineSpace = currentSdk >= 22 ? 0 : U.getDisplayUtils().dip2px(5);//行间距需求5.0版本适配
    ...
    private void setMedalIconPrefix(List<String> medalList) {
        if (medalList != null && !medalList.isEmpty()) {
            for (int index = 0; index < medalList.size(); index++) {
                ...
                if (drawable != null) {
                    drawable.setBounds(0, 0, (int) (drawable.getIntrinsicWidth() * U.getDisplayUtils().getDensity()),
                            (int) (drawable.getIntrinsicHeight() * U.getDisplayUtils().getDensity()));
...
//        barrageTv.setPadding(0, 0, 16, 3);
    ...
    public void setLikeDrawable(Drawable drawable) {
        ...
        commentSpan.append("a");
//        int currentSdk = Build.VERSION.SDK_INT;
//        int lineSpace = currentSdk >= 20 ? 0 : U.getDisplayUtils().dip2px(5);//行间距需求5.0版本适配
            ...
            if (nameClickSpan == null) {
                nameClickSpan = new SpanUtils.MyClickableSpan(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (!liveComment.isCanClickable() || U.getCommonUtils().isFastDoubleClick(1000)) {
                            return;
                        }
