...
import static com.wali.live.modulewatch.barrage.model.barrage.BarrageMsg.RoomTxtMessageExt.MSG_SMART_NORMAL;
...
public class CommentModel implements Comparable<CommentModel> {
    ...
    @ColorRes
    private int commentColor = R.color.primary_content;
    @ColorRes
    private int nameColor = R.color.primary_name;
...
//        }
    ...
    public boolean isNoble() {
        return this.nobleLevel == UserInfoConstans.NOBLE_LEVEL_FIFTH || this.nobleLevel == UserInfoConstans.NOBLE_LEVEL_FOURTH
                || this.nobleLevel == UserInfoConstans.NOBLE_LEVEL_THIRD || this.nobleLevel == UserInfoConstans.NOBLE_LEVEL_SECOND
                || this.nobleLevel == UserInfoConstans.NOBLE_LEVEL_TOP;
    }
    ...
    public static CommentModel loadFromBarrage(BarrageMsg msg, boolean isGameLive) {
        ...
        if (isGameLive) {
            liveComment.setNameColor(R.color.game_barrage_name);
            liveComment.setCommentColor(R.color.game_barrage_content);
        } else {
            liveComment.setCommentColor(R.color.normal_barrage_name);
            liveComment.setNameColor(R.color.normal_barrage_content);
        }
...
//                MyLog.w("ForbiddenMsg"+msg.toString());
            ...
            case BarrageMsgType.B_MSG_TYPE_SET_MANAGER: {
                liveComment.setBody(U.app().getString(R.string.barrage_set_manager_body), false);
                liveComment.setLevel(0);
                liveComment.setCertificationType(0);
                liveComment.setName(null);
            }
            ...
            case BarrageMsgType.B_MSG_TYPE_CANCEL_MANAGER: {
                liveComment.setBody(U.app().getString(R.string.barrage_cancel_manager_body), false);
                liveComment.setLevel(0);
                liveComment.setCertificationType(0);
                liveComment.setName(null);
            }
            ...
            case BarrageMsgType.B_MSG_TYPE_ANNOUNCEMENT:
                liveComment.setCommentColor(R.color.barrage_announcement);
                liveComment.setBody(msg.getBody(), false);
                liveComment.setLevel(0);
                liveComment.setCertificationType(0);
                liveComment.setName(null);
            ...
            case BarrageMsgType.B_MSG_TYPE_ROOM_FOUCES_ANCHOR: {
                if (!TextUtils.isEmpty(name)) {
                    liveComment.setBody(U.app().getString(R.string.barrage_focus_body), false);
                } else {
                    liveComment.setName(null);
                    liveComment.setBody(msg.getBody(), false);
                }
                if (isGameLive) {
                    liveComment.setCommentColor(R.color.barrage_game_theme);
                } else {
                    liveComment.setCommentColor(R.color.barrage_normal_theme);
                }
            }
            break;
            case BarrageMsgType.B_MSG_TYPE_LEAVE: {
                if (!TextUtils.isEmpty(name)) {
                    liveComment.setBody(U.app().getString(R.string.barrage_leave_live_body), false);
                } else {
                    liveComment.setName(null);
                    liveComment.setBody(msg.getBody(), false);
                }
            ...
            case BarrageMsgType.B_MSG_TYPE_JOIN: {
                if (!TextUtils.isEmpty(name)) {
                    ...
                    if (msg.getVipLevel() >= 2 && !msg.isVipFrozen() && !msg.isVipHide()) {
                        ...
                        if (!TextUtils.isEmpty(welcomeVipEnterRoomTip)) {
                            ...
                            if (isGameLive) {
                                liveComment.setCommentColor(R.color.game_barrage_name_vipjoin);
                                liveComment.setNameColor(R.color.game_barrage_content_vipjoin);
                            }
                            break;
                        }
                    }

                    String s = U.app().getString(R.string.barrage_enter_live_body);
                    if (msg.getMsgExt() instanceof BarrageMsg.JoinRoomMsgExt) {
                        ...
                        if (ext.type == BarrageMsg.JoinRoomMsgExt.TYPE_NEARBY) {
                            s = U.app().getString(R.string.barrage_enter_live_by_near_body);
                        }
                    }
                ...
                liveComment.setNameColor(R.color.barrage_name_join);
                if (isGameLive) {
                    liveComment.setCommentColor(R.color.game_barrage_content_join);
                }
            }
            break;
            case BarrageMsgType.B_MSG_TYPE_LIKE: {
                if (!TextUtils.isEmpty(name)) {
                    liveComment.setBody(U.app().getString(R.string.barrage_like_body), false);
                } else {
                    liveComment.setName(null);
                    liveComment.setBody(msg.getBody(), false);
                }
                liveComment.setCommentColor(R.color.barrage_like);
            }
            break;
            ...
            case BarrageMsgType.B_MSG_TYPE_SHARE: {
                if (!TextUtils.isEmpty(name)) {
                    liveComment.setBody(U.app().getString(R.string.barrage_share_body), false);
                } else {
                    liveComment.setName(null);
                    liveComment.setBody(msg.getBody(), false);
                }
            ...
            case BarrageMsgType.B_MSG_TYPE_ROOM_BACKGROUND_GIFT: {
                ...
                if (isGameLive) {
                    liveComment.setNameColor(R.color.background_gift);
                    liveComment.setCommentColor(R.color.barrage_game_theme);
                } else {
                    liveComment.setCommentColor(R.color.barrage_normal_theme);
                }
            ...
            case BarrageMsgType.B_MSG_TYPE_LIGHT_UP_GIFT:
                ...
                liveComment.setCommentColor(R.color.light_up_gift);
                break;
            case BarrageMsgType.B_MSG_TYPE_PAY_BARRAGE:
            case BarrageMsgType.B_MSG_TYPE_TEXT: {
                String body = msg.getBody();
                if (msg.isRedName()) {
                    liveComment.setCommentColor(R.color.barrage_name_default);
                    liveComment.setLevel(0);
                    ...
                    switch (ext.getType()) {
                        case BarrageMsg.RoomTxtMessageExt.MSG_TYPE_NORMAL:
                            try {
                                com.wali.live.proto.LiveMessage.AtMessage atMessage = com.wali.live.proto.LiveMessage.AtMessage.parseFrom(ext.getExt().toByteArray());
                                if (atMessage.hasAtUser()
                                        && atMessage.getAtUser() == UserAccountManager.getInstance().getUuidAsLong()) {
                                    if (isGameLive) {
                                        liveComment.setCommentColor(R.color.barrage_game_text);
                                    } else {
                                        liveComment.setCommentColor(R.color.barrage_normal_text);// TODO: 17-6-5
                                    }
                                }
                            } catch (Exception e) {
                    ...
                    if (isGameLive) {
                        liveComment.setCommentColor(R.color.black_trans_80);
                    } else {
                        liveComment.setCommentColor(R.color.white);
                    }
                }
                if (msg.getNobleLevel() >= UserInfoConstans.NOBLE_LEVEL_FOURTH) {
                    liveComment.setNameColor(R.color.barrage_noble_fourth);
                }
                if (msg.getNobleLevel() >= UserInfoConstans.NOBLE_LEVEL_FIFTH) {
...
//                liveComment.setName(name + ": ");
    ...
    private static String getWelcomeVipEnterRoomTip() {
        String[] tips = U.app().getResources().getStringArray(R.array.welcome_vip_enter_room_tip);
