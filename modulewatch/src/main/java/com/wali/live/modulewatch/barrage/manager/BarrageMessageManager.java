package com.wali.live.modulewatch.barrage.manager;
...
import com.common.milink.callback.MiLinkPacketDispatcher;
...
import com.wali.live.modulewatch.barrage.event.BarrageMsgEvent;
...
import com.wali.live.modulewatch.barrage.model.barrage.SystemMsgModel;
...
public class BarrageMessageManager implements MiLinkPacketDispatcher.PacketDataHandler {
    ...
    public boolean processPacketData(PacketData data) {
        ...
        switch (data.getCommand()) { //JDK7之后支持字符串
            ...
            case MiLinkCommand.COMMAND_RADIO_SIGNAL_PUSH: {
                processPushBarrage(data, false, MiLinkCommand.COMMAND_RADIO_SIGNAL_PUSH);
            }
            break;
        }
        return false;
    ...
    private void processPushBarrage(PacketData data, boolean careLeaveRoom, String command) {
        try {
            PushMessage pushMsg = PushMessage.parseFrom(data.getData());
            if (pushMsg == null) {
                return;
            }
            ...
            for (Message msg : pushMsg.getMessageList()) {
                if (msg == null) {
                    continue;
                }
...
//                        GiftManager.popupGlobalGiftMsgWindow(barrageMsg, (BarrageMsg.RoomTxtMessageExt) barrageMsg.getMsgExt(), RoomInfoGlobalCache.getsInstance().getCurrentRoomId());
    ...
    private void updateUserInfo(Message msg) {
        MyUserInfo myUserInfo = MyUserInfoManager.getInstance().getMyUserInfo();
        if (msg.getFromUserLevel() != myUserInfo.getLevel()) {
            myUserInfo.setLevel(msg.getFromUserLevel());
            MyLog.w(TAG + " updateUserInfo and level is " + msg.getFromUserLevel());
        }
        if (msg.getNobleLevel() != myUserInfo.getNobleLevel()) {
            myUserInfo.setNobleLevel(msg.getNobleLevel());
        }
        if (msg.getVipLevel() > myUserInfo.getVipLevel()
                || msg.getVipDisable() != myUserInfo.getIsVipFrozen()
                || msg.getVipHidden() != myUserInfo.getIsVipHide()) {
            myUserInfo.setVipLevel(msg.getVipLevel());
            myUserInfo.setIsVipFrozen(msg.getVipDisable());
            myUserInfo.setIsVipHide(msg.getVipHidden());
        }
        // 如果是进入房间消息，尝试矫正一下nickname ，这里谨防变成系统消息，这里暂时不考虑英文版
        if (msg.getMsgType() == BarrageMsgType.B_MSG_TYPE_JOIN
                && TextUtils.isEmpty(myUserInfo.getNickName())
                && !TextUtils.isEmpty(msg.getFromUserNickName())
                && !"系统消息".equals(msg.getFromUserNickName())) {
            myUserInfo.setNickName(msg.getFromUserNickName());
        }
        MyUserInfoManager.getInstance().setMyUserInfo(myUserInfo);
    }
...
//        msg1.setBody("欢迎" + MyUserInfoManager.getInstance().getNickname() + " 来到主播" + roomBaseDataModel.getNickName() + "的甜蜜派对");
    ...
    public void pretendPushBarrage(BarrageMsg msg) {
        // VIP相关
        // todo 暂时屏蔽
//        msg.setVipLevel(MyUserInfoManager.getInstance().getUser().getVipLevel());
//        msg.setVipFrozen(MyUserInfoManager.getInstance().getUser().getIsVipFrozen());
//        msg.setVipHide(MyUserInfoManager.getInstance().getUser().getIsVipHide());
//        msg.setNobleLevel(MyUserInfoManager.getInstance().getUser().getNobleLevel());
//        MyLog.d(TAG, "pretendPushBarrage nobelType=" + MyUserInfoManager.getInstance().getUser().getNobleLevel());
//
...
//        sendRecvEvent(barrageMsgList);
    ...
    private BarrageMsg addMedalInfo(BarrageMsg msg) {
        //设置用户勋章-
        // 暂时屏蔽
//        if ((MyUserInfoManager.getInstance().getUser().getBeforeNickNameMedalList() != null && !MyUserInfoManager.getInstance().getBeforeNickNameMedalList().isEmpty()) ||
...
//        return msg;
    ...
    private void processSystemMessage(PacketData packetData) {
        if (packetData != null) {
            try {
                SyncSysMsgResponse response = SyncSysMsgResponse.parseFrom(packetData.getData());
    ...
    public void sendBarrageMessageAsync(BarrageMsg msg, boolean needReSend) {
        if (msg != null) {
            ...
            if (ext != null) {
                builder.setMsgExt(okio.ByteString.of(ext.toByteArray()));
            }
            ...
            if (msg.getGlobalRoomMessageExt() != null) {
                ...
                List<com.wali.live.proto.LiveMessage.InnerGlobalRoomMessageExt> innerList = new ArrayList<>();
                if (msg.getGlobalRoomMessageExt().getInnerGlobalRoomMessageExtList() != null) {
                    for (int i = 0; i < msg.getGlobalRoomMessageExt().getInnerGlobalRoomMessageExtList().size(); i++) {
                        if (msg.getGlobalRoomMessageExt().getInnerGlobalRoomMessageExtList().get(i).getType() != BarrageMsg.INNER_GLOBAL_VFAN) {
                            innerList.add(new InnerGlobalRoomMessageExt.Builder().setType(msg.getGlobalRoomMessageExt().getInnerGlobalRoomMessageExtList().get(i).getType()).build());
                        }
                    }
                }
                builder2.addAllInnerGlobalRoomMsgExt(innerList);
                builder.setGlobalRoomMsgExt(builder2.build());
            }
            if (!TextUtils.isEmpty(msg.getOpponentRoomId())) {

                PKRoomInfo.Builder pkRoomInfoBuilder = new PKRoomInfo.Builder();
    ...
    public boolean sendBarrageMessageSync(BarrageMsg msg, int timeout) {
        if (msg != null) {
            ...
            packetData.setData(builder.build().toByteArray());
            PacketData result = MiLinkClientAdapter.getInstance().sendSync(packetData, timeout);
            if (result != null) {
                try {
                    RoomMessageResponse response = RoomMessageResponse.parseFrom(result.getData());
                    if (response != null) {
    ...
    public void sendHuYaBarrageMessage(HuyaSendMessageReq huyaSendMessageReq) {
        if (huyaSendMessageReq != null) {
            if (huyaSendMessageReq.fromUid <= 0 || huyaSendMessageReq.anchorHuyaUid <= 0) {
    ...
    public void sendSyncSystemMessage(SyncSysMsgRequest syncSystemMessageRequest) {
        if (syncSystemMessageRequest != null) {
            if (syncSystemMessageRequest.from_user <= 0 || TextUtils.isEmpty(syncSystemMessageRequest.room_id)
                    || syncSystemMessageRequest.cid <= 0) {
                MyLog.w(TAG, "send chaMessageReadRequest from or to or cid is null,so cancel");
                return;
            }
            PacketData packetData = new PacketData();
    ...
    public static void checkBarrageMsgStatus() {
        if (mSendingMsgCache != null && mSendingMsgCache.size() > 0) {
            ...
            for (Long key : keys) {
                ...
                if (barrageMsg != null && System.currentTimeMillis() - barrageMsg.getSenderMsgId() > MIN_RESPONSE_TIME_OUT_CHECK_TIME && barrageMsg.getResendTimes() < MAX_RETRY_SEND_TIMS && MiLinkClientAdapter.getInstance().isMiLinkLogined()) {
