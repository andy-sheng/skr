...
import com.mi.liveassistant.proto.RedEnvelProto;
...
//                        msgExt = new ForbiddenMsgExt.ShopMessageExt(shopMessage);
//                        break;
//                    }
    ...
    public static class ForbiddenMsgExt implements MsgExt {
        ...
        public String getBanMessage(long anchorId, long uuid, int msgType, String senderName) {
            ...
            if (TextUtils.isEmpty(banNickname))
                return message;
            if (senderName == null || senderName.equals(banNickname) || senderName.equals("系统消息"))
                senderName = "";
            String operator = "";
            //判断当前用户是否是主播
            if (anchorId == uuid) {
            ...
            if (msgType == BarrageMsgType.B_MSG_TYPE_FORBIDDEN) {
                if (TextUtils.isEmpty(operator)) {
                    message = String.format("%s已被禁言", banNickname);
                } else {
                    message = String.format("%s已被%s禁言", banNickname, operator);
                }
            } else if (msgType == BarrageMsgType.B_MSG_TYPE_CANCEL_FORBIDDEN) {
                if (TextUtils.isEmpty(operator)) {
                    message = String.format("%s被取消禁言", banNickname);
                } else {
                    message = String.format("%s被%s取消禁言", banNickname, operator);
                }
            }
            return message;
        }
...
//    }
    ...
    public static class MsgRuleChangeMessageExt implements MsgExt {
        private MessageRuleModel messageRule;

        public MessageRuleModel getMessageRule() {
            return messageRule;
        }

        public MsgRuleChangeMessageExt(LiveMessageProto.MsgRuleChangeMessage msgRuleChangeMessage) {
            messageRule = new MessageRuleModel(msgRuleChangeMessage.getMsgRule());
        }
