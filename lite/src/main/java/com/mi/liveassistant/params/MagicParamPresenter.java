package com.mi.liveassistant.params;
...
import com.mi.liveassistant.proto.CloudParamsProto;
...
public class MagicParamPresenter extends BaseParamPresenter {
    ...
    public MagicParamPresenter(@NonNull Context context) {
        super(context);
    }
        ...
        mFaceBeautySub = Observable.just(0)
                ...
                .subscribe(new Action1<Integer>() {
                    ...
                    public void call(Integer errCode) {
                        MyLog.w(TAG, "syncMagicParams errCode=" + errCode);
                    }
    ...
    public static class MagicParams {
        ...
        private void parseBeauty(String beautyStr) {
            String[] ret = beautyStr.substring(1, beautyStr.length() - 1).split(",");
            if (ret == null || ret.length == 0) {
                MyLog.e(TAG, "parseBeauty failed, beautyStr=" + beautyStr);
                return;
            }
            if (beauty.length != ret.length) {
                beauty = new int[ret.length];
            }
            for (int i = 0; i < ret.length; ++i) {
                beauty[i] = Integer.valueOf(ret[i].trim());
            }
        }

        public void loadParams(@NonNull Context context) {
            String codeStr = PreferenceUtils.getSettingString(KEY_MAGIC_PARAM_SUPPORT_CODE, "");
            try {
                JSONObject jsonObject = new JSONObject(codeStr);
                parseBeauty(jsonObject.getString("beauty"));
                isFilter = jsonObject.getBoolean("filter");
//                isExpression = !PlusParamUtils.isHideExpression(true);
        ...
        private void saveParams(@NonNull Context context) {
            try {
                PreferenceUtils.setSettingString(KEY_MAGIC_PARAM_SUPPORT_CODE, new JSONObject()
                        .put("beauty", Arrays.toString(beauty).toString())
                        .put("filter", isFilter).toString());
