...
public class FoldView extends RelativeLayout {
    ...
    private static final int LEFT_PADDING = DisplayUtils.dip2px(10);
    private static final int TOP_PADDING = DisplayUtils.dip2px(10);
    ...
    public void addContentView(View variableLengthView) {
        ...
        variableLengthView.setId(CommonUtils.generateViewId());
        LayoutParams lp = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        addView(variableLengthView, lp);
        mVariableLengthView = (IFoldViewContent) variableLengthView;
        View view = inflate(getContext(), R.layout.fold_view_layout, null);
    ...
    private void updateBottomHint() {
        if (mState == STATE_FOLD) {
            mHint.setText(getContext().getResources().getString(R.string.fold_view_show_all));
            mArrow.setBackground(getContext().getResources().getDrawable(R.drawable.down_arrow_bg));
        } else if (mState == STATE_UNFOLD) {
            mHint.setText(getContext().getResources().getString(R.string.pack_up));
