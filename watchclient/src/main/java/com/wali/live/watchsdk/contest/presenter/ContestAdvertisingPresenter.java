...
public class ContestAdvertisingPresenter extends BaseRxPresenter<IContestAdvertisingView>{
    ...
    public ContestAdvertisingPresenter(IContestAdvertisingView view, Context context) {
        super(view);
        mParentContext = context;
        mDownloadModel = new MyDownloadModel();
    }
    ...
    public void startDownload() {
        ...
        mDownloadModel.setCardByDownloadApp(false);
        updateState(State.StartDownload);
        DownloadManager.Request request = new DownloadManager.Request(Uri.parse(mDownloadModel.getDownloadURL()));
        MyLog.w(TAG, "gameName=" + mDownloadModel.getName());
        request.setTitle(mDownloadModel.getName());
        request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);

        String filename = mDownloadModel.getAppId() + "_" + System.currentTimeMillis() + ".apk";
    ...
    public boolean hasCardByDownloadApp() {
        return mDownloadModel.hasCardByDownloadApp();
    }

    public boolean hasCardByOpenApp() {
        return mDownloadModel.hasCardByOpenApp();
    }
    ...
    private void tryLaunchAPk() {
        mDownloadModel.setCardByOpenAPP(false);
        Intent intent = mParentContext.getPackageManager().getLaunchIntentForPackage(mDownloadModel.getPackageName());
        if (intent != null) {
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            mParentContext.startActivity(intent);
        } else {
            MyLog.w(TAG, "intent launch fail, packageName=" + mDownloadModel.getPackageName());
        }
        updateState(State.Launch);
    }

    public void initState() {
        mHasInstalled = PackageUtils.isInstallPackage(mDownloadModel.getPackageName());
        if (mHasInstalled) {
    ...
    private void registerInstallReceiver() {
        ...
        mInstallReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                if (intent != null && mDownloadModel != null) {
                    String action = intent.getAction();
                    String packageName = intent.getData().getSchemeSpecificPart();
                    MyLog.d(TAG, "intent packageName=" + packageName + ";modelPackageName=" + mDownloadModel.getPackageName());
                    if (packageName.equals(mDownloadModel.getPackageName())) {
