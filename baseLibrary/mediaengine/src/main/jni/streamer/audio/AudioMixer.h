...
public:
    ...
    void setInputVolume(int idx, float vol) {mInputVolume[idx] = vol;}
    void setOutputVolume(float vol) {mOutputVolume = vol;}
    void setMute(bool mute) {mMute = mute;}
    void setBlockingMode(bool blockingMode) {mBlockingMode = blockingMode;}
    int config(int idx, int sampleRate, int channels, int bufferSamples,
...
private:
    ...
    ChannelFifo* fifoInit(int sampleRate, int channels, int bufferSamples, int fifoSizeInMs);
    void fifoRelease(ChannelFifo* channelFifo);
    int mixAll(uint8_t* inBuf, int inSize);
    void mix(short* src1, int size1, float vol1, short* src2, int size2, float vol2);
};
