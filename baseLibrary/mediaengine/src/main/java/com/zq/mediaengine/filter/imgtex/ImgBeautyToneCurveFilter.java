...
/**
 *  Image beauty filter from photoshop acv file.
 */
...
public class ImgBeautyToneCurveFilter extends ImgTexFilter {
    ...
    private int[] mToneCurveTexture = new int[]{ ImgTexFrame.NO_TEXTURE};
    ...
    public ImgBeautyToneCurveFilter(GLRender glRender) {
        super(glRender, GlUtil.BASE_VERTEX_SHADER,
                TONE_CURVE_FRAGMENT_SHADER);

        PointF[] defaultCurvePoints = new PointF[]{
                new PointF(0.0f, 0.0f), new PointF(0.5f, 0.5f),
    ...
    public void onInitialized() {
        mToneCurveTextureUniformLocation = GLES20.glGetUniformLocation(mProgramId, "toneCurveTexture");
        mToneCurveTexture[0] = ImgTexFrame.NO_TEXTURE;
        updateCurve();
    }
    ...
    protected void onDrawArraysPre() {
        ...
        if (mToneCurveTexture[0] != ImgTexFrame.NO_TEXTURE) {
    ...
    private void updateToneCurveTexture() {
        if (mUpdateTexture) {
            ...
            if ((mRedCurve.size() >= 256) && (mGreenCurve.size() >= 256) &&
                    (mBlueCurve.size() >= 256) && (mRgbCompositeCurve.size() >= 256)) {
                byte[] toneCurveByteArray = new byte[256 * 4];
                for (int currentCurveIndex = 0; currentCurveIndex < 256; currentCurveIndex++) {
                    // BGRA for upload to texture
                    toneCurveByteArray[currentCurveIndex * 4 + 2] = (byte) ((int) Math.min(Math.max(currentCurveIndex +
                            mBlueCurve.get(currentCurveIndex) + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
                    toneCurveByteArray[currentCurveIndex * 4 + 1] = (byte) ((int) Math.min(Math.max(currentCurveIndex +
                            mGreenCurve.get(currentCurveIndex) + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
                    toneCurveByteArray[currentCurveIndex * 4] = (byte) ((int) Math.min(Math.max(currentCurveIndex +
                            mRedCurve.get(currentCurveIndex) + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
                    toneCurveByteArray[currentCurveIndex * 4 + 3] = (byte) (255 & 0xff);
                }

                mToneCurveTexture[0] = GlUtil.loadTexture(ByteBuffer.wrap(toneCurveByteArray),
                        256, 1, mToneCurveTexture[0]);
            }
    ...
    private ArrayList<Float> createSplineCurve(PointF[] points) {
        ...
        for (Point newPoint : splinePoints) {
            ...
            float distance = (float) Math.sqrt(Math.pow((origPoint.x - newPoint.x), 2.0) +
