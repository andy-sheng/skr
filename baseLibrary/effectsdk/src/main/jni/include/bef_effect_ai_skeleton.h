...
#define BEF_MAX_SKELETON_POINT_NUM 18
#define BEF_MAX_SKELETON_NUM  2


typedef struct bef_skeleton_point_info {
    float x; // 对应 cols, 范围在 [0, width] 之间
    float y; // 对应 rows, 范围在 [0, height] 之间
    bool is_detect; // 如果该值为 0, 则 x,y 无意义
} bef_skeleton_point_info;
...
/**
 * @brief Set skeleton network size 设置骨骼的网络大小
 *                      设置的大小，建议值128， 224，可以使用 96, 168
 *                      需要按照 128/224相同比例的设置参数，
 *                      设置参数影响性能
 * @param [in] handle Created skeleton handle
 *                      已创建的骨骼句柄
 * @param [in] width Skeleton detect network width
 *                      骨骼检测的网络宽度
 ...
 */
BEF_SDK_API bef_effect_result_t
bef_effect_ai_skeleton_setparam(
                               bef_effect_handle_t handle,
                               int width,
                               int height
                               );
