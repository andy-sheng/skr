...
public class DefaultDownLoaderImpl implements DownloadListener, DownLoadResultListener {
    ...
    private void onDownloadStartInternal(String url, String contentDisposition, String mimetype, long contentLength) {
        ...
        if (this.mPermissionListener != null) {
            if (this.mPermissionListener.intercept(url, AgentWebX5Permissions.STORAGE, "download")) {
                return;
            }
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if ((mList = checkNeedPermission()).isEmpty()) {
                ...
                mAction.setPermissions(AgentWebX5Permissions.STORAGE);
    ...
    private List<String> checkNeedPermission() {
        ...
        for (int i = 0; i < AgentWebX5Permissions.STORAGE.length; i++) {

            if (ContextCompat.checkSelfPermission(mActivityWeakReference.get(), AgentWebX5Permissions.STORAGE[i]) != PackageManager.PERMISSION_GRANTED) {
                deniedPermissions.add(AgentWebX5Permissions.STORAGE[i]);
            }
        }
    ...
    private void performDownload(String url, long contentLength, File file) {
        ...
        //并行下载.
        if (isParallelDownload.get()) {
            new RealDownLoader(new DownLoadTask(NoticationID++, url, this, isForce, enableIndicator, mContext, file, contentLength, mDownLoadMsgConfig, icon == -1 ? R.drawable.download : icon)).executeOnExecutor(ExecutorProvider.getInstance().provide(), (Void[]) null);
        } else {
            //默认串行下载.
            new RealDownLoader(new DownLoadTask(NoticationID++, url, this, isForce, enableIndicator, mContext, file, contentLength, mDownLoadMsgConfig, icon == -1 ? R.drawable.download : icon)).execute();
        }
