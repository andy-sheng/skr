...
//        AC.inst().init(getGlobalConfigModule(mModules));
        ...
        U.app().registerActivityLifecycleCallbacks(activityLifecycle);
        ...
        U.app().registerActivityLifecycleCallbacks(activityLifecycleForRxLifecycle);
        ...
        for (Application.ActivityLifecycleCallbacks lifecycle : mActivityLifecycles) {
            U.app().registerActivityLifecycleCallbacks(lifecycle);
        }

        mComponentCallback = new AppComponentCallbacks();

        //注册回掉: 内存紧张时释放部分内存
        U.app().registerComponentCallbacks(mComponentCallback);
    ...
    public void onTerminate(@NonNull Application application) {
        if (mComponentCallback != null) {
            U.app().unregisterComponentCallbacks(mComponentCallback);
        }
        if (mActivityLifecycles != null && mActivityLifecycles.size() > 0) {
            for (Application.ActivityLifecycleCallbacks lifecycle : mActivityLifecycles) {
                U.app().unregisterActivityLifecycleCallbacks(lifecycle);
            }
        }
        if (mAppLifecycles != null && mAppLifecycles.size() > 0) {
            for (AppLifecycles lifecycle : mAppLifecycles) {
                lifecycle.onTerminate(U.app());
            }
        }
     ...
     */
    private GlobalParams getGlobalConfigModule(List<ConfigModule> modules) {

        GlobalParams.Builder builder = GlobalParams
                .builder();
