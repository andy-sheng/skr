...
import com.indexrecyclerview.adapter.IndexableFooterAdapter;
import com.indexrecyclerview.adapter.IndexableHeaderAdapter;
import com.indexrecyclerview.comparator.InitialComparator;
import com.indexrecyclerview.comparator.PinyinComparator;
import com.indexrecyclerview.model.EntityWrapper;
import com.indexrecyclerview.model.IndexableEntity;
import com.indexrecyclerview.observer.DataObserver;
import com.indexrecyclerview.observer.HeaderFooterDataObserver;
import com.indexrecyclerview.observer.IndexBarDataObserver;
...
public class IndexableLayout extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        PADDING_RIGHT_OVERLAY = U.getDisplayUtils().dip2px(80);
    ...
    private <T extends IndexableEntity> ArrayList<EntityWrapper<T>>  transform(final List<T> datas) {
        try {
            ...
            for (int i = 0; i < datas.size(); i++) {
                ...
                String pinyin = U.getPinyinUtils().getPingYin(indexName);
                entity.setPinyin(pinyin);

                // init EntityWrapper
                if (U.getPinyinUtils().matchingLetter(pinyin)) {
                    entity.setIndex(pinyin.substring(0, 1).toUpperCase());
                    entity.setIndexByField(item.getFieldIndexBy());
                } else if (U.getPinyinUtils().matchingPolyphone(pinyin)) {
                    entity.setIndex(U.getPinyinUtils().gePolyphoneInitial(pinyin).toUpperCase());
                    entity.setPinyin(U.getPinyinUtils().getPolyphoneRealPinyin(pinyin));
