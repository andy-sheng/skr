...
import com.common.rxretrofit.utils.CookieDbUtil;
...
public class CookieInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (cache) {
            ...
            CookieResulte resulte = CookieDbUtil.queryCookieBy(url);
            long time = System.currentTimeMillis();
            /*保存和更新本地数据*/
            if (resulte == null) {
                resulte = new CookieResulte(url, bodyString, time);
                CookieDbUtil.saveCookie(resulte);
            } else {
                resulte.setResulte(bodyString);
                resulte.setTime(time);
                CookieDbUtil.updateCookie(resulte);
            }
        }
        return response;
    }
