    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                MyLog.w("Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
            } catch (InterruptedException | ExecutionException e) {
                MyLog.e("Error pinging server due to unexpected error", e);
            }
            attempts++;
            timeout *= 2;
        }
        ...
        MyLog.e(error, new ProxyCacheException(error));
        return false;
    }
