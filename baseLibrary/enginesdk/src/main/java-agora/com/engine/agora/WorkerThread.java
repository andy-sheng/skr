...
public class WorkerThread extends Thread {
    ...
    public WorkerThread() {
        ...
        this.mEngineEventHandler = new AgoraEngineCallbackWithLog(TAG);
    }

    private RtcEngine ensureRtcEngineReadyLock() {
        if (mRtcEngine == null) {
            try {
                mRtcEngine = RtcEngine.create(mContext, "9D8CA2B17EC04DBE82D95F1FAFE7250E", mEngineEventHandler.mRtcEventHandler);
                //mRtcEngine.setParameters("{\"rtc.log_filter\": 65535}");
            } catch (Exception e) {
                MyLog.d(Log.getStackTraceString(e));
...
//                    + File.separator + mContext.getPackageName() + "/log/agora-rtc.log");
    ...
    public final void joinChannel(final String channel, int uid) {
        if (Thread.currentThread() != this) {
            MyLog.d("joinChannel() - worker thread asynchronously " + channel + " " + (uid & 0xFFFFFFFFL));
