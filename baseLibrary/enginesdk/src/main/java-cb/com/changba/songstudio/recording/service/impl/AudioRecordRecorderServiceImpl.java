...
public class AudioRecordRecorderServiceImpl implements RecorderService {
	...
	class RecordThread implements Runnable {
		...
		@Override
		public void run() {
			int scoringBufferMaxSize = bufferSizeInBytes;
			byte[] audioSamples = new byte[scoringBufferMaxSize];
...
//				android.util.Log.d(TAG, "RecordThread run() localPaused="+localPaused+"  isUnAccom="+isUnAccom);
				...
				int audioSampleSize = getAudioRecordBufferByByte(
...
//							audioSampleSize);
	...
	protected int getAudioRecordBufferByByte(int scoringBufferMaxSize,
									   byte[] audioSamples) {
		if (audioRecord != null) {
			int size = audioRecord.read(audioSamples, 0,
					scoringBufferMaxSize);
