...
#include "../Plugin_conv_reverb/smallhamon.h"
#include "../Plugin_conv_reverb/jucam.h"
#include "../Plugin_conv_reverb/lowshower.h"
#include "../Plugin_conv_reverb/shower.h"
#include "../Plugin_conv_reverb/smokeybar.h"
#include "../Plugin_conv_reverb/thetrackingroom.h"
#include "../Plugin_conv_reverb/cd.h"
#include "../Plugin_conv_reverb/tandc.h"
...
void CovReverbV2Calcu_API(CovReverbV2_s *mReverb)
{
	...
	icov = 0;
	have = 0;
	mReverb->fstdelay_n = 0;
	mReverb->h0 = 1;
	mReverb->h0R = 1;
	switch(mReverb->reverbkind)//these IRs are all at 48khz
	{
	case CD:
		mReverb->fstdelay_n = 1294;
		
		preverbhL = g_cdC441 + mReverb->fstdelay_n;
		preverbhR = g_cdC441 + mReverb->fstdelay_n;
		reverbLlen = 29000 - mReverb->fstdelay_n;//9000..
		reverbRlen = 29000 - mReverb->fstdelay_n;//9000
		have = 1;
		implusegain_1 = 0.0001;

		mReverb->h0 = g_cdC441[0] * implusegain_1;
		mReverb->h0R = g_cdC441[0] * implusegain_1;

		mReverb->mDelay.front = 0;
		mReverb->mDelay.rear = mReverb->mDelay.front + mReverb->fstdelay_n*mReverb->channelin;
	...
	{
		assert(mReverb->have == 1);
		assert(icov == 0);
		mReverb->h0 += mReverb->dry;
		mReverb->h0R += mReverb->dry;
		for (i = 0; i < mfirblen; i++)
		{
			mfirb[i] *= mReverb->wet;
			mfirbR[i] *= mReverb->wet;
...
void CovReverbV2Run_API(CovReverbV2_s *mReverb, short *input, int inlen, short *output)
{
	short y1[SKR_MAX_FRAME_SAMPLE_STEREO];
	short y2[SKR_MAX_FRAME_SAMPLE_STEREO];
	short inputdelayed[SKR_MAX_FRAME_SAMPLE_STEREO];
	int i;
	...
	if (mReverb->fstdelay_n == 0)
	{
		if (mReverb->have == 1)
		{
			...
			if (mReverb->channelin == 2 && mReverb->channelout == 2 && mReverb->xsame == 1)
			{
				ChanelConvert(1, 2, inlen, input, NULL, y1, y2);
				FastCovVerylonghRun_API(&mReverb->mfir, y1, inlen / 2, y1);
				ChanelConvert(2, 1, inlen / 2, y1, y1, output, NULL);
			}
			else
			{
	...
	{
		assert(mReverb->have == 1);
		BufDelayRun_API_loop(&mReverb->mDelay, input, inlen, inputdelayed);
		if (mReverb->channelin == 2 && mReverb->channelout == 2 && mReverb->xsame == 1)
		{
			ChanelConvert(1, 2, inlen, inputdelayed, NULL, y1, y2);
			FastCovVerylonghRun_API(&mReverb->mfir, y1, inlen / 2, y1);
			ChanelConvert(2, 1, inlen / 2, y1, y1, inputdelayed, NULL);
		}
		else
		{
			FastCovVerylonghRun_API(&mReverb->mfir, inputdelayed, inlen, inputdelayed);
		}

		if (mReverb->h0 == mReverb->h0R)
		{
			for (i = 0; i < inlen; i++)
			{
				output[i] = stoshort(inputdelayed[i] + input[i]*mReverb->h0);
			}
		} 
		else
		{
