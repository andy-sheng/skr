...
static void
SKRiir_xDyD_2(short *output, short *input, int inlen, float *mem_output, short *mem_input, float a,
              float g, int D) {
    short i, j;
    float sumx, sumy;
    short *x;
...
static void
SKRiir_xyD_Dn_2(short *output, short *input, int inlen, float *mem_output, short *mem_input,
                float a, float g, float b, int *D, int maxD) {
    short i, j;
    float sumx, sumy;
    short *x;
    float *y;
...
void EchoRun_API(Echo_s *mEcho, short *input, int inlen, short *output) {
    ...
    if (mEcho->channel == 1) {
        ...
        if (mEcho->Echo_depth_ms == 0) {
            ...
            SKRiir_xDyD_2(output, input, inlen, mEcho->memoutputL, mEcho->meminputL, mEcho->Echo_a,
                          mEcho->Echo_g, mEcho->Echo_D);
        }
        ...
        if (mEcho->Echo_depth_ms == 0) {
            //assert(mEcho->Echo_b == 1);//we just run echo mod
            SKRiir_xDyD_2(x16L, x16L, inRlen, mEcho->memoutputL, mEcho->meminputL, mEcho->Echo_a,
                          mEcho->Echo_g, mEcho->Echo_D);
            SKRiir_xDyD_2(x16R, x16R, inRlen, mEcho->memoutputR, mEcho->meminputR, mEcho->Echo_a,
                          mEcho->Echo_g, mEcho->Echo_D);
        }
