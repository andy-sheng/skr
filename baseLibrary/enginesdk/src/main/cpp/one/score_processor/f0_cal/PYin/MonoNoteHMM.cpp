...
//                tempProb_b = std::pow(minDistProb, par.yinTrust) *
//                           boost::math::pdf(pitchDistr_b[i],
//                                            pitchProb[minDistCandidate].first);
                tempProb = std::pow(minDistProb, par.yinTrust) *
                            distribution::pdf(pitchDistr[i],
                                 pitchProb[minDistCandidate].first);
//                if(tempProb_b != tempProb) {
//                    printf("why tempProb_b is %.5f tempProb is %.5f", tempProb_b, tempProb);
//                }
            } else {
                tempProb = 1;
            }
...
MonoNoteHMM::build()
{
    ...
    {
//        pitchDistr_b.push_back(boost::math::normal(0,1));
        pitchDistr.push_back(distribution::Normal(0,1));
...
//        pitchDistr_b[index] = boost::math::normal(mu, par.sigmaYinPitchAttack);
//        pitchDistr_b[index+1] = boost::math::normal(mu, par.sigmaYinPitchStable);
//        pitchDistr_b[index+2] = boost::math::normal(mu, 1.0); // dummy
        pitchDistr[index] = distribution::Normal(mu, par.sigmaYinPitchAttack);
        pitchDistr[index+1] = distribution::Normal(mu, par.sigmaYinPitchStable);
        pitchDistr[index+2] = distribution::Normal(mu, par.sigmaYinPitchInter); // change by zhouyu
    }
    
//    boost::math::normal noteDistanceDistr_b(0, par.sigma2Note);
    distribution::Normal noteDistanceDistr(0, par.sigma2Note);
    ...
    for (size_t iPitch = 0; iPitch < (par.nS * par.nPPS); ++iPitch)
    {
        ...
        {
            int fromPitch = int(iPitch);
            int toPitch = int(jPitch);
...
//                double tempWeightSilent_b = boost::math::pdf(noteDistanceDistr_b,
//                                                           semitoneDistance);
                double tempWeightSilent = distribution::pdf(noteDistanceDistr,
                                                           semitoneDistance);
//                if(tempWeightSilent_b != tempWeightSilent) {
//                    printf("Why");
//                }
...
{
    return pitchDistr[index].m_mean;
}

double
MonoNoteHMM::getFrequency(size_t index)
{
    return 440 * pow(2, (pitchDistr[index].m_mean-69)/12);
}
