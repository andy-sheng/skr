...
//    }
    double tmp[m_nPitch];memset(tmp, 0, m_nPitch*sizeof(double));
    double *outPtr = &out[0];
    CMathUtil::MultiArrayByScale(outPtr, tmp, (probReallyPitched/probYinPitched), int(m_nPitch));
    double pdivp=(1 - probReallyPitched) / m_nPitch;
    for (size_t iPitch = 0; iPitch < m_nPitch; ++iPitch)
    {
        if (probYinPitched > 0) out[iPitch] =tmp[iPitch] ;
        out[iPitch+m_nPitch] = pdivp;
    }
...
MonoPitchHMM::build()
{
    ...
    for (size_t iPitch = 0; iPitch < m_nPitch; ++iPitch)
    {
        ...
        int minNextPitch = iPitch>int(m_transitionWidth/2) ? int(iPitch-m_transitionWidth/2) : 0;
        int maxNextPitch = iPitch<int(m_nPitch-m_transitionWidth/2) ? int(iPitch+m_transitionWidth/2) : int(m_nPitch-1);
