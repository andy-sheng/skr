...
void MVRecordingPreviewController::pushToRTCService() {
    LOGI("MVRecordingPreviewController::pushToRTCService");
    JNIEnv *env;
    ...
    if (NULL != jcls) {
        jmethodID pushToRTCServiceFun = env->GetMethodID(jcls, "pushToRTCService", "(I)V");
        if (NULL != pushToRTCServiceFun) {
            env->CallVoidMethod(obj, pushToRTCServiceFun, renderer->getInputTexId());
        }
    }
