...
public class LocalBroadcastExprEditor extends ExprEditor {
    ...
    def private replaceStatement(MethodCall call) {
        ...
        if (method == 'getInstance') {
             ...
             * 变成了
             * PluginLocalBroadcastManager.sendBroadcast(PluginLocalBroadcastManager.getInstance(PDMainAcitivity.this), localIntent);
             *
             * 这里分解
             * LocalBroadcastManager a = LocalBroadcastManager.getInstance(PDMainAcitivity.this) ==> PluginLocalBroadcastManager.getInstance(PDMainAcitivity.this)
             * a.sendBroadcast(intent) ==> PluginLocalBroadcastManager.sendBroadcast(a,intent)  $0 代表 原方法中的调用方 注意是原方法
             */
