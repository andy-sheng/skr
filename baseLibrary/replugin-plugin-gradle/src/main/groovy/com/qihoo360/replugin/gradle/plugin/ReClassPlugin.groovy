...
public class ReClassPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        println "${AppConstant.TAG} Welcome to replugin  plugin world ! "
        ...
        if (isApp) {
            ...
            /**
             * 遍历android extension的Application variants 组合。
             * android gradle 插件，会对最终的包以多个维度进行组合。
             * ApplicationVariant的组合 = {ProductFlavor} x {BuildType} 种组合
             */
            android.applicationVariants.all { variant ->
                ...
                /**
                 * 获取assemble task(即打包apk的task)，后续的task需要依赖此task，
            ...
            /**
             * Transform 是 Android Gradle API ，
             * 允许第三方插件在class文件转为dex文件前操作编译完成的class文件，
             * 这个API的引入是为了简化class文件的自定义操作而无需对Task进行处理。
             * 在做代码插桩时，本质上是在merge{ProductFlavor}{BuildType}Assets Task 之后，
             * transformClassesWithDexFor{ProductFlavor}{BuildType} Transform 之前,
             * 插入一个transformClassesWith{YourTransformName}For{ProductFlavor}{BuildType} Transform，
             * 此Transform中完成对class文件的自定义操作
             * （包括修改父类继承，方法中的super方法调用，方法参数替换等等，这个class交给你，理论上是可以改到怀疑人生）。
             *
             */
