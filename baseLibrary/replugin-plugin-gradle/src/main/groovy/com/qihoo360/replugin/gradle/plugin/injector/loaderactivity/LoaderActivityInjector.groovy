...
public class LoaderActivityInjector extends BaseInjector {
    ...
    private def handleActivity(ClassPool pool, String activity, String classesDir) {
        ...
        println ">>> handleActivity $activity"
...
*/
            ...
            println ">>> handleActivity 需要替换的类为 $ctCls.name 他的父类为 $superCls.name"
            ...
            if (superCls != null) {
                 ...
                 * 注：当CtClass 调用writeFile()、toClass()、toBytecode() 这些方法的时候，
                 * Javassist会冻结CtClass Object，将不允许对CtClass object进行修改
                 *
                 */
                if (ctCls.isFrozen()) {
                    ctCls.defrost()
                ...
                // 修改声明的父类后，还需要方法中所有的 super 调用。 javaassist 库
...
//                                    origMethodCall = origMethodCall + "System.out.println();;";
//
//                                call.replace(origMethodCall);
