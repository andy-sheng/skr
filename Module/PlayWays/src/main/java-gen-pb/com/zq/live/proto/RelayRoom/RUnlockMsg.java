// Code generated by Wire protocol buffer compiler, do not edit.
// Source file: relay_room.proto
package com.zq.live.proto.RelayRoom;

import com.squareup.wire.FieldEncoding;
import com.squareup.wire.Message;
import com.squareup.wire.ProtoAdapter;
import com.squareup.wire.ProtoReader;
import com.squareup.wire.ProtoWriter;
import com.squareup.wire.WireField;
import com.squareup.wire.internal.Internal;
import java.io.IOException;
import java.lang.Boolean;
import java.lang.Object;
import java.lang.Override;
import java.lang.String;
import java.lang.StringBuilder;
import java.util.List;
import okio.ByteString;

public final class RUnlockMsg extends Message<RUnlockMsg, RUnlockMsg.Builder> {
  public static final ProtoAdapter<RUnlockMsg> ADAPTER = new ProtoAdapter_RUnlockMsg();

  private static final long serialVersionUID = 0L;

  public static final Boolean DEFAULT_ENABLENOLIMITDURATION = false;

  /**
   * 解锁信息
   */
  @WireField(
      tag = 1,
      adapter = "com.zq.live.proto.RelayRoom.RUserLockInfo#ADAPTER",
      label = WireField.Label.REPEATED
  )
  private final List<RUserLockInfo> userLockInfo;

  /**
   * 开启没有限制的持续时间
   */
  @WireField(
      tag = 2,
      adapter = "com.squareup.wire.ProtoAdapter#BOOL"
  )
  private final Boolean enableNoLimitDuration;

  public RUnlockMsg(List<RUserLockInfo> userLockInfo, Boolean enableNoLimitDuration) {
    this(userLockInfo, enableNoLimitDuration, ByteString.EMPTY);
  }

  public RUnlockMsg(List<RUserLockInfo> userLockInfo, Boolean enableNoLimitDuration,
      ByteString unknownFields) {
    super(ADAPTER, unknownFields);
    this.userLockInfo = Internal.immutableCopyOf("userLockInfo", userLockInfo);
    this.enableNoLimitDuration = enableNoLimitDuration;
  }

  @Override
  public Builder newBuilder() {
    Builder builder = new Builder();
    builder.userLockInfo = Internal.copyOf("userLockInfo", userLockInfo);
    builder.enableNoLimitDuration = enableNoLimitDuration;
    builder.addUnknownFields(unknownFields());
    return builder;
  }

  @Override
  public boolean equals(Object other) {
    if (other == this) return true;
    if (!(other instanceof RUnlockMsg)) return false;
    RUnlockMsg o = (RUnlockMsg) other;
    return unknownFields().equals(o.unknownFields())
        && userLockInfo.equals(o.userLockInfo)
        && Internal.equals(enableNoLimitDuration, o.enableNoLimitDuration);
  }

  @Override
  public int hashCode() {
    int result = super.hashCode;
    if (result == 0) {
      result = unknownFields().hashCode();
      result = result * 37 + userLockInfo.hashCode();
      result = result * 37 + (enableNoLimitDuration != null ? enableNoLimitDuration.hashCode() : 0);
      super.hashCode = result;
    }
    return result;
  }

  @Override
  public String toString() {
    StringBuilder builder = new StringBuilder();
    if (!userLockInfo.isEmpty()) builder.append(", userLockInfo=").append(userLockInfo);
    if (enableNoLimitDuration != null) builder.append(", enableNoLimitDuration=").append(enableNoLimitDuration);
    return builder.replace(0, 2, "RUnlockMsg{").append('}').toString();
  }

  public byte[] toByteArray() {
    return RUnlockMsg.ADAPTER.encode(this);
  }

  public static final RUnlockMsg parseFrom(byte[] data) throws IOException {
    RUnlockMsg c = null;
       c = RUnlockMsg.ADAPTER.decode(data);
    return c;
  }

  /**
   * 解锁信息
   */
  public List<RUserLockInfo> getUserLockInfoList() {
    if(userLockInfo==null){
        return new java.util.ArrayList<RUserLockInfo>();
    }
    return userLockInfo;
  }

  /**
   * 开启没有限制的持续时间
   */
  public Boolean getEnableNoLimitDuration() {
    if(enableNoLimitDuration==null){
        return DEFAULT_ENABLENOLIMITDURATION;
    }
    return enableNoLimitDuration;
  }

  /**
   * 解锁信息
   */
  public boolean hasUserLockInfoList() {
    return userLockInfo!=null;
  }

  /**
   * 开启没有限制的持续时间
   */
  public boolean hasEnableNoLimitDuration() {
    return enableNoLimitDuration!=null;
  }

  public static final class Builder extends Message.Builder<RUnlockMsg, Builder> {
    private List<RUserLockInfo> userLockInfo;

    private Boolean enableNoLimitDuration;

    public Builder() {
      userLockInfo = Internal.newMutableList();
    }

    /**
     * 解锁信息
     */
    public Builder addAllUserLockInfo(List<RUserLockInfo> userLockInfo) {
      Internal.checkElementsNotNull(userLockInfo);
      this.userLockInfo = userLockInfo;
      return this;
    }

    /**
     * 开启没有限制的持续时间
     */
    public Builder setEnableNoLimitDuration(Boolean enableNoLimitDuration) {
      this.enableNoLimitDuration = enableNoLimitDuration;
      return this;
    }

    @Override
    public RUnlockMsg build() {
      return new RUnlockMsg(userLockInfo, enableNoLimitDuration, super.buildUnknownFields());
    }
  }

  private static final class ProtoAdapter_RUnlockMsg extends ProtoAdapter<RUnlockMsg> {
    public ProtoAdapter_RUnlockMsg() {
      super(FieldEncoding.LENGTH_DELIMITED, RUnlockMsg.class);
    }

    @Override
    public int encodedSize(RUnlockMsg value) {
      return RUserLockInfo.ADAPTER.asRepeated().encodedSizeWithTag(1, value.userLockInfo)
          + ProtoAdapter.BOOL.encodedSizeWithTag(2, value.enableNoLimitDuration)
          + value.unknownFields().size();
    }

    @Override
    public void encode(ProtoWriter writer, RUnlockMsg value) throws IOException {
      RUserLockInfo.ADAPTER.asRepeated().encodeWithTag(writer, 1, value.userLockInfo);
      ProtoAdapter.BOOL.encodeWithTag(writer, 2, value.enableNoLimitDuration);
      writer.writeBytes(value.unknownFields());
    }

    @Override
    public RUnlockMsg decode(ProtoReader reader) throws IOException {
      Builder builder = new Builder();
      long token = reader.beginMessage();
      for (int tag; (tag = reader.nextTag()) != -1;) {
        switch (tag) {
          case 1: builder.userLockInfo.add(RUserLockInfo.ADAPTER.decode(reader)); break;
          case 2: builder.setEnableNoLimitDuration(ProtoAdapter.BOOL.decode(reader)); break;
          default: {
            FieldEncoding fieldEncoding = reader.peekFieldEncoding();
            Object value = fieldEncoding.rawProtoAdapter().decode(reader);
            builder.addUnknownField(tag, fieldEncoding, value);
          }
        }
      }
      reader.endMessage(token);
      return builder.build();
    }

    @Override
    public RUnlockMsg redact(RUnlockMsg value) {
      Builder builder = value.newBuilder();
      Internal.redactElements(builder.userLockInfo, RUserLockInfo.ADAPTER);
      builder.clearUnknownFields();
      return builder.build();
    }
  }
}
