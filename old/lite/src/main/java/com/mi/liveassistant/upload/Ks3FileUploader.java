    ...
    public boolean startUpload() {
        if (null == mAtt || TextUtils.isEmpty(mAtt.getLocalPath())) {
            if (mUploadCallBack != null) {
                mUploadCallBack.onPostExecute(false, "file_upload_failed_path_error", getResourceId());
            }
            return false;
        }
        ...
        if (!file.exists() || !file.isFile() || file.length() == 0) {
            if (mUploadCallBack != null) {
                mUploadCallBack.onPostExecute(false, "file_upload_failed_file_error", getResourceId());
            }
            return false;
        }
